...
The `latent_logit_lens` path takes the final captured `output_last_position_hidden_state`, applies `final_norm` when present, projects through `lm_head`, and computes a continuous source-answer minus target-answer token logit margin. BF-Patch and BF-Swap records now expose `clean_score_diagnostic`, `patched_score_diagnostic`, `score_failure_reasons`, `margin_source`, `continuous_margin_shift`, `effective_margin_shift`, and `latent_logit_margin_shift`, while retaining legacy `logprob_margin_shift` / `swap_margin_shift`. Main-facing BF summaries use `continuous_margin_shift`: aligned generation scores first, latent logit-lens second, and no parsed-answer fallback in the primary continuous scalar.
